...
def _refresh_keep_alive(BUY_TIME):
    buy_time_object = datetime.datetime.strptime(BUY_TIME, '%Y-%m-%d %H:%M:%S')
    now_time = datetime.datetime.now()
    if (buy_time_object-now_time).seconds <0:
        print("请指定未来时间")
        exit(0)
    if (buy_time_object-now_time).seconds >180:
        driver.get("https://www.taobao.com")
        time.sleep(60)
        return False
    else:
...
def buy(url,to_send,BUY_TIME):
    while _refresh_keep_alive(BUY_TIME) :
        #打开购物车
        driver.get(url)
        time.sleep(2)
        flag=True
...
if __name__=="__main__":
   #url=input("请输入商品链接:")
   url="https://m.tb.cn/h.VcRBKqg?sm=ef5e3c"
   #BUY_TIME=input("几点开始轮候抢购:")
   BUY_TIME="2020-02-19 18:12:00"
   #to_send=input("请输入通知邮箱:")
   to_send="122568712@qq.com"
   login()
   buy(url,to_send,BUY_TIME)